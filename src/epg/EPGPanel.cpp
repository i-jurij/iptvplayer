...
void EPGPanel::SetupUI() {
  ...
  rightSizer->Add(m_channelNameLabel, 0, wxALL, 5);
  ...
  navSizer->Add(m_dateLabel, 0, wxALIGN_CENTER_VERTICAL | wxRIGHT, 10);
  navSizer->Add(m_prevDayBtn, 0, wxRIGHT, 2);
  navSizer->Add(m_todayBtn, 0, wxRIGHT, 2);
  navSizer->Add(m_nextDayBtn, 0);
  navSizer->AddStretchSpacer(1);
  rightSizer->Add(navSizer, 0, wxEXPAND | wxLEFT | wxRIGHT | wxBOTTOM, 5);
  ...
  rightSizer->Add(m_programList, 1, wxEXPAND | wxALL, 5);
  ...
  detailSizer->Add(m_detailTitle, 0, wxALL, 5);
  detailSizer->Add(m_detailDesc, 1, wxEXPAND | wxLEFT | wxRIGHT | wxBOTTOM, 5);
  rightSizer->Add(detailSizer, 0, wxEXPAND | wxALL, 5);
  ...
  bottomBtnSizer->Add(m_refreshBtn, 0, wxRIGHT, 5);
  bottomBtnSizer->Add(m_manageSourcesBtn, 0);
  rightSizer->Add(bottomBtnSizer, 0, wxALIGN_LEFT | wxALL, 5);
...
void EPGPanel::OnRefreshEPG(wxCommandEvent &) {
  if (m_epgManager) {
    ...
    ShowMessage("EPG refresh started in background.");
  }
}
...
void EPGPanel::OnManageSources(wxCommandEvent &) {
  ...
  if (mf) {
    ...
    ShowMessage("Open Settings → EPG to manage sources.");
  }
}
...
void EPGPanel::LoadProgramsForChannel(const std::string &channelId,
  ...
  if (!m_epgManager) {
    m_programList->InsertItem(0, "EPG manager not available");
    return;
  }
  if (!m_epgManager->IsLoaded()) {
    auto sources = m_epgManager->GetSources();
    if (sources.empty()) {
      m_programList->InsertItem(
          0, "No EPG sources configured. Add sources in Settings → EPG.");
    } else {
      m_programList->InsertItem(0, "EPG not loaded yet. Try Refresh.");
    }
    return;
  }
  ...
  if (programs.empty()) {
    m_programList->InsertItem(0, "No programs for this date");
    return;
  }
  ...
  for (const auto &prog : programs) {
    ...
    // Форматирование без явного преобразования — используем локальное время по
    // умолчанию
...
void EPGPanel::OnEPGUpdated(wxCommandEvent &) {
  if (!m_currentChannelId.empty()) {
    LoadProgramsForChannel(m_currentChannelId, m_currentDate);
  }
}
